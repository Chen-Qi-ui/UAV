...
float cal_gyro0,cal_gyro1,cal_gyro2;
// 获取真正的微秒时间
uint64_t get_time_usec_precise(void) {
    // 假设系统时钟为 168MHz
    return (uint64_t)DWT->CYCCNT / (168000000/ 1000000);
}
...
void vSensor_ReceiveTask(void *pvParameters)
{
    ...
    const float ba_prime_mag[3] = {-4.740319, -66.844692, 16.311883};

    const float Ka_mag[3] = {0.016005, 0.013936, 0.015183};
    ...
    for(;;)
    {
        ...
        if(xbmi_Status == pdPASS || xak8975_Status == pdPASS)
        {
            ...
            cal_gyro0 = cal_gyro[0];
			cal_gyro1 = cal_gyro[1];
			cal_gyro2 = cal_gyro[2];
...
// 将滤波后的值传给队列或 EKF
          ...
          Cal_Data.mag[2] = lpf_mz;
            mavlink_msg_highres_imu_pack(
                1, MAV_COMP_ID_AUTOPILOT1, &msg_sensor, get_time_usec_precise(),
